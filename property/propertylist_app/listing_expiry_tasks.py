...
def _qa_mode() -> bool:
    ...
    # Staging historically has not always declared ENVIRONMENT, so recognise
    # the configured staging host as a safe fallback. Production URLs do not
    # contain the word "staging".
...
def listing_expiry_warning_sweep() -> int:
    ...
    Production: warn when an active advert is within 7 days of paid_until.
    QA/staging: a paid cycle lasts 20 minutes and warning is sent at minute 15.
    """
    now = timezone.now()
...
def listing_expiry_sweep() -> int:
    ...
    Production uses paid_until as the 30-day entitlement boundary.
    QA/staging compresses the same paid cycle to 20 minutes from the latest
    successful listing/relisting payment.
    """
    now = timezone.now()
    today = timezone.localdate()
    qa_mode = _qa_mode()
    ...
    for room in rooms:
        ...
        # Natural advert expiry is NOT a manual unpublish. Keep raw lifecycle
        # status active and make the payment entitlement expired. The API then
        # derives listing_state="expired", so the web card belongs only in
        # Ads Expired and cannot appear in public search.
        if qa_mode:
        ...
        if _notifications_allowed(owner):
            ...
            # Keep the original paid period in the dedupe/context even after
            # QA moves paid_until into the past.
