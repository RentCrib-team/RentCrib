...
def test_booking_creation_signal_keeps_synchronous_database_work_bounded(
    django_user_model,
):
    ...
    # Booking creation must stay a bounded request-path operation. Required
    # side effects remain synchronous DB records, but repeated thread/profile/
    # template lookups must not grow into a large query fan-out.
    assert len(queries) <= 12

    landlord_notification = Notification.objects.get(
        user=landlord,
